...
namespace {
    /**
     * Benchmark for Watchdog N+1 Performance with STALE sessions
     * Usage: php tests/benchmark_watchdog_stale.php
     */
    ...
    function wp_upload_dir() {
        return ['basedir' => sys_get_temp_dir() . '/ap_watchdog_bench_stale'];
    }
    ...
    function delete_transient($transient) {
        global $mock_transients, $query_count;
        $query_count++; // Increment query count for delete
        usleep(500); // Simulate delete latency
    ...
    class MockWPDB {
        ...
        public function query($query) {
            global $query_count;
            $query_count++;
            // Simulate DB latency for delete query
            usleep(500);
            return true;
        }
    ...
    function setup_bench_env($count) {
        ...
        $dir = sys_get_temp_dir() . '/ap_watchdog_bench_stale';
        if (is_dir($dir)) {
            // clean up old
        ...
        for ($i = 0; $i < $count; $i++) {
            ...
            // Create a STALE transient for this session
            // Stale means updated_at is older than STALE_THRESHOLD (24 hours)
            $mock_transients[Watchdog::SESSION_TRANSIENT_PREFIX . $uploadId] = [
                'upload_id' => $uploadId,
                'created_at' => time() - (60 * 60 * 25), // 25 hours ago
                'updated_at' => time() - (60 * 60 * 25),
            ];
        }
    }
    ...
    echo "Setting up environment with $itemCount STALE sessions...\n";
    setup_bench_env($itemCount);

    echo "Running Watchdog benchmark (stale)...\n";
    ...
    echo "Queries (get_transient / delete_transient / DB calls): " . $query_count . "\n";
